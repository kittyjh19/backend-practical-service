...
본 서비스는 현재 전역 예외 처리(`@ControllerAdvice`)가 적용되어 있지 않아,  
에러 응답은 **실제 코드 분석을 기반으로 한 논리적 분류**입니다.

### 에러 유형

- **400 Bad Request**
  - 요청 값이 유효하지 않은 경우
  - 요청 데이터로 처리할 수 없는 상태인 경우

- **403 Forbidden**
  - 게시글 작성자가 아닌 사용자가 수정 또는 삭제를 시도한 경우

- **404 Not Found**
  - 존재하지 않는 게시글을 조회/수정/삭제하려는 경우
  - 존재하지 않는 사용자 ID로 요청한 경우
...
- 실제 HTTP 상태 코드는 전역 예외 처리 구현 여부에 따라 달라질 수 있습니다.
- 현재 단계에서는 예외 발생 지점과 비즈니스 로직을 기준으로 에러를 분류하였습니다.
...
* 자동 문서화 도구(Swagger)는 추후 배포 단계에서 적용을 고려하고 있습니다.
